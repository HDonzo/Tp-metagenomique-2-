...
def find_start(start_regex, sequence, start, stop) -> Union[int, None]:
    ...
    """
    codon = start_regex.search(sequence,start,stop)
...
def has_shine_dalgarno(shine_regex, sequence, start, max_shine_dalgarno_distance) :
    ...
    """
    end_cod = start - 6
    start_cod = start - max_shine_dalgarno_distance
    if start_cod < 0 : 
        return False
    match_shine = shine_regex.search(sequence, start_cod, end_cod)
    if match_shine : 
        return True
    return False
...
def predict_genes(sequence, start_regex, stop_regex, shine_regex, 
    ...
    while len(sequence)-position >= min_gap : 
        position = find_start(start_regex,sequence,position,len(sequence))
        if position : 
            stop = find_stop(stop_regex,sequence,position)
            if stop and (stop - position + 1) > min_gene_len : 
                if has_shine_dalgarno(shine_regex, sequence,position,max_shine_dalgarno_distance) :
                    predict_list.append([position + 1, stop + 3])
